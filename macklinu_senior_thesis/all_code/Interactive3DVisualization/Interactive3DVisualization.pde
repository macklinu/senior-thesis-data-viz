...
void draw() {
  ...
  if (!show3D) {
    // 2D data visualization would go here
    stroke(0);
    strokeWeight(2);
    textSize(48);
...
void keyPressed() {
  // pressing left and right arrow keys will navigate through preset views of the dataset
  if (key == CODED) {
    if (keyCode == LEFT) {
      int test = cur;
  ...
  // press 's' to save your current view to a properties file
  if (key == 's' || key == 'S') {
  ...
  if (key == 'h' || key == 'H') showGUI = !showGUI; // show/hide GUI
  if (key == '3') show3D = !show3D; // switch from 2D to 3D
  if (key == 'c' || key == 'C') save("screenshots/" + timestamp() + ".png"); // take screenshot
}

// table used to store camera position values
